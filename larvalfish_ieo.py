...
class LarvalFish_sardine(OceanDrift):
    ...
    def update_terminal_velocity(self, Tprofiles=None,
        ...
        eggsize = self.elements.diameter  # 0.0014 for NEA Cod		# HAY QUE CAMBIAR EL DIAMETRO
	...
	#DENS_sp = 0.78797*ini_water_density+4.2519
        ...
        self.elements.egg_dens[eggs] = self.dens_growth(self.elements.stage_fraction, DENSsp)[eggs]	# CAMBIO
    ...
    def update_fish_larvae(self):
        ...
        if len(eggs) > 0:
            ...
            days_in_timestep = self.time_step.total_seconds()/(60.*60.)  # The fraction of a day completed in one time step, MODIFICADO, ANTES DIVIDIA ENTRE 24, AHORA YA NO, PORQUE LA amd_duration viene dada en horas y asi anulamos unidades
        ...
        if len(larvae) > 0 or len(larvae2) > 0:
            ...
            self.elements.length[np.where(self.elements.hatched==2)[0]] += (coef_1 + coef_2*self.environment.sea_water_temperature[np.where(self.elements.hatched == 2)[0]])*food*0.5/(food + 0.1)*self.time_step.total_seconds()/86400.
    ...
    def larvae_vertical_migration(self):
        # Pendiente de mejorar en caso de uso. Solamente es la base para los condicionales, pero no 
        # tiene en cuenta el efecto de la turbulencia, solo fija las posiciones a dichas alturas.
        if len(np.where(self.elements.hatched==2)[0]) == 0:
            return
        
        #indices para cada fase
        index_1 = np.where((self.elements.hatched == 2) & (self.elements.length > 4.5) & (self.elements.length <= 6.))[0]
        index_2 = np.where((self.elements.hatched == 2) & (self.elements.length > 6.) & (self.elements.length <= 8.))[0]
        index_3 = np.where((self.elements.hatched == 2) & (self.elements.length > 8.) & (self.elements.length <= 10.))[0]
        index_4 = np.where((self.elements.hatched == 2) & (self.elements.length > 10.) & (self.elements.length <= 12.))[0]
        #print(index_1)
        #print(index_2)
        #print(index_3)
        #print(index_4)
        ...
        if self.time.hour < 12:
            self.elements.z[index_1] = -11.89*np.ones(len(index_1))
            self.elements.z[index_2] = -8.92*np.ones(len(index_2))
            self.elements.z[index_3] = -8.26*np.ones(len(index_3))
            self.elements.z[index_4] = -7.56*np.ones(len(index_4))
        else:     
            self.elements.z[index_1] = -13.5*np.ones(len(index_1))
            self.elements.z[index_2] = -8.43*np.ones(len(index_2))
            self.elements.z[index_3] = -8.06*np.ones(len(index_3))
            self.elements.z[index_4] = -8.49*np.ones(len(index_4))
